...
function GameScreen({t,settings,difficulty:initDiff,savedState,seed:initSeed,onHome,onSave,onWin,onLoss,isDaily,isPractice,gameMode}){
  ...
  return(
    <div style={{position:"fixed",inset:0,background:gt.bg,backgroundImage:settings.bgPhoto?`url(${settings.bgPhoto})`:"none",backgroundSize:"cover",backgroundPosition:"center",display:"flex",flexDirection:isLandscape?"row":"column",alignItems:"center",boxSizing:"border-box",fontFamily:MONO,color:gt.pri,touchAction:"manipulation",overflow:"hidden"}}>
      ...
      {/* AD BANNER — top of screen, hidden in landscape or if ads removed */}
      {!isLandscape&&!settings._noAds&&<div style={{width:"100%",height:50,background:gt.bgCard,borderBottom:`1px solid ${gt.brd}`,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,paddingTop:"max(env(safe-area-inset-top),0px)"}}>
        {/* Ad banner slot */}
      </div>}
