    ...
    <div style={{position:"fixed",inset:0,background:t.bg,display:"flex",flexDirection:"column",alignItems:"center",paddingBottom:0,boxSizing:"border-box",fontFamily:MONO,overflow:"hidden"}}>
      ...
      <div style={{width:"100%",maxWidth:460,flex:1,display:"flex",flexDirection:"column",padding:"0 14px",overflowY:"auto",paddingBottom:70}}>
        ...
        {navTab==="home"&&<>
          ...
          <div style={{...an(0.08),marginBottom:10}}>
            <div style={{background:t.bgCard,border:`1.5px solid ${t.brd}`,borderRadius:14,padding:"12px 14px"}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
                <span style={{fontSize:10,color:t.mut,fontFamily:MONO,letterSpacing:"0.1em",textTransform:"uppercase"}}>This Week</span>
                <span style={{fontSize:10,color:t.acc,fontFamily:MONO,fontWeight:700}}>{"🔥 "+streak+" day streak"}</span>
              </div>
              <div style={{display:"flex",gap:4,justifyContent:"space-between"}}>
                {(()=>{
                  ...
                  return days.map((_,i)=>{
                    ...
                    const done=arch.includes(ds)||stats?.lastDailyWin===ds;
                    const future=d>today;
                    return(
                      <div key={i} style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",gap:3}}>
                        <div style={{fontSize:8,color:isToday?t.acc:t.mut,fontFamily:MONO,fontWeight:isToday?700:400}}>{days[i]}</div>
                        <div style={{width:22,height:22,borderRadius:"50%",background:done?"#4ade80":isToday?`${t.acc}22`:future?t.bg:t.bgHov,border:`1.5px solid ${done?"#4ade80":isToday?t.acc:t.brd}`,display:"flex",alignItems:"center",justifyContent:"center"}}>
          ...
          {(()=>{
            ...
            return tip?(
              <div style={{...an(0.09),marginBottom:10}}>
                <div style={{background:`${t.acc}10`,border:`1px solid ${t.acc}33`,borderRadius:12,padding:"10px 14px",display:"flex",gap:10,alignItems:"flex-start"}}>
                  <span style={{fontSize:14,flexShrink:0}}>💡</span>
                  <div>
                    <div style={{fontSize:9,color:t.acc,fontFamily:MONO,letterSpacing:"0.1em",marginBottom:3}}>TIP OF THE DAY</div>
                    <div style={{fontSize:"clamp(11px,2.8vw,12px)",color:t.sec,lineHeight:1.5}}>{tip.label}</div>
                  </div>
                </div>
              </div>
            ):null;
...
function GameScreen({t,settings,difficulty:initDiff,savedState,seed:initSeed,onHome,onSave,onWin,onLoss,isDaily,isPractice,gameMode}){
  ...
  const [numHigh,setNumHigh]=useState(null);
  const [numLock,setNumLock]=useState(null);
  const [correctPop,setCorrectPop]=useState(null);
  ...
  const lastTap=useRef({n:null,t:0});
  ...
  const doNumber=useCallback((num)=>{
    ...
    if(nx.every((rr,ri)=>rr.every((v,ci)=>v===sol[ri][ci]))){
      ...
      const pck=`${r}-${c}`;setPop(p=>({...p,[pck]:true}));setTimeout(()=>setPop(p=>{const n={...p};delete n[pck];return n;}),350);
      chkPulse(nx,r,c);
      // If numLock active, deselect cell so next tap picks a new one
      if(numLock!==null)setSel(null);
    }
  },[sel,won,over,sol,board,given,noteBoard,notes,mistakes,settings.autoNotes,settings.highlightMistakes,
     pushUndo,timer,isFocus,isZen,isPractice,sfx,hap,doWin,endLv,chkPulse,numLock]);
  ...
  return(
    <div style={{position:"fixed",inset:0,background:gt.bg,backgroundImage:settings.bgPhoto?`url(${settings.bgPhoto})`:"none",backgroundSize:"cover",backgroundPosition:"center",display:"flex",flexDirection:isLandscape?"row":"column",alignItems:"center",justifyContent:"center",boxSizing:"border-box",fontFamily:MONO,color:gt.pri,touchAction:"manipulation",overflow:"hidden",paddingTop:!isLandscape&&!settings._noAds?"calc(max(env(safe-area-inset-top),0px) + 50px)":"0",paddingBottom:"max(env(safe-area-inset-bottom),12px)"}}>
      <style>{`@keyframes shk{0%{transform:translateX(0)}20%{transform:translateX(-3px)}40%{transform:translateX(3px)}60%{transform:translateX(-2px)}80%{transform:translateX(2px)}100%{transform:translateX(0)}} @keyframes pop{0%{transform:scale(1)}40%{transform:scale(1.18)}100%{transform:scale(1)}} @keyframes plsL{0%,100%{opacity:1}50%{opacity:0.35}} @keyframes cfFall{0%{transform:translateY(-10px) rotate(0deg);opacity:1}100%{transform:translateY(100vh) rotate(720deg);opacity:0}} @keyframes cfSpin{0%{transform:translateY(-10px) rotate(0deg) scaleX(1);opacity:1}50%{transform:translateY(50vh) rotate(360deg) scaleX(-1);opacity:0.8}100%{transform:translateY(100vh) rotate(720deg) scaleX(1);opacity:0}}`}</style>
            ...
            return(
              <div key={ck}
                onTouchEnd={e=>{e.preventDefault();if(won||over)return;
                  ...
                  if(numLock!==null&&!isGiv&&val===0){setSel([r,c]);setTimeout(()=>doNumber(numLock),0);return;}
                ...
                {val!==0
                  ?<span style={{fontSize:numSz,fontWeight:600,color:nc,fontFamily:activeNumFont,fontVariantNumeric:"tabular-nums",lineHeight:1,opacity:isFad?0:1,transition:"opacity .6s",animation:pop[ck]?"pop .35s ease":isFlsh&&!isFad?"shk .35s ease":"none",display:"block",textAlign:"center",width:"100%",position:"relative",zIndex:1}}>
                      {val}
      ...
      <div style={{display:"flex",flexDirection:"column",gap:isLandscape?8:0,alignItems:"center",justifyContent:isLandscape?"center":"flex-start",flexShrink:0,width:isLandscape?"min(44vw,320px)":"min(96vw,440px)",padding:isLandscape?"12px 8px":"0"}}>
          ...
          return(
            <Btn key={n} onTap={()=>{}} disabled={done||won||over}
              ...
              onTouchEnd={e=>{e.preventDefault();e.stopPropagation();if(!done&&!won&&!over){if(lpTimers.current[n]){clearTimeout(lpTimers.current[n]);lpTimers.current[n]=null;
                if(sel){
                  const[sr,sc]=sel;
                  const cellVal=board[sr][sc];
                  if(cellVal===0||cellVal===n){doNumber(n);}
                  else{setSel(null);setNumHigh(v=>v===n?null:n);}
                }else{
                  // double-tap same number toggles numLock
                  if(numLock===n){setNumLock(null);}
                  else if(numHigh===n){setNumLock(n);setNumHigh(null);}
                  else{setNumLock(null);setNumHigh(v=>v===n?null:n);}
                }
              }}}}
              ...
              {n}
              {isLocked&&<span style={{position:"absolute",top:2,left:3,fontSize:"clamp(5px,1.2vw,7px)",color:"#f59e0b",lineHeight:1}}>🔒</span>}
              {!done&&<span style={{position:"absolute",bottom:2,right:3,fontSize:"clamp(6px,1.4vw,8px)",color:gt.mut,lineHeight:1}}>{9-cnt}</span>}
            </Btn>
          );
        })}
        <Btn onTap={()=>{if(sel)doNumber(0);else{setSel(null);setNumHigh(null);setNumLock(null);}}} disabled={won||over} style={{flex:1,aspectRatio:"0.88",border:`1.5px solid ${gt.brdM}`,borderRadius:10,background:gt.bgCard,color:gt.mut,fontSize:"clamp(14px,4vw,20px)",display:"flex",alignItems:"center",justifyContent:"center",minWidth:0,position:"relative"}}>
          ⌫
          {undos.length>0&&<span style={{position:"absolute",top:2,right:3,fontSize:"clamp(5px,1.2vw,7px)",color:gt.mut,lineHeight:1}}>{undos.length}</span>}
        </Btn>
      </div>
      ...
      {won&&(
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.88)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:100,backdropFilter:"blur(8px)",padding:20,animation:"pgIn 320ms cubic-bezier(0.22,1,0.36,1) both"}}>
          <div style={{background:gt.bgCard,border:`2px solid ${acc}`,borderRadius:20,padding:"28px 20px",textAlign:"center",boxShadow:`0 0 80px ${acc}40`,width:"min(92vw,360px)",animation:"winCard 380ms cubic-bezier(0.34,1.56,0.64,1) both"}}>
            ...
            {(()=>{
              const prevBest=nav?.savedState?.prevBest;
              const isPB=prevBest&&timer<prevBest;
              return isPB?<div style={{color:"#f59e0b",fontSize:11,fontWeight:700,marginBottom:4,fontFamily:MONO}}>🏆 Personal Best! -{fmt(prevBest-timer)}</div>:null;
            })()}
...
// STORE SCREEN
// ─────────────────────────────────────────────
function Store({t,onBack,onPurchase,onTryTheme}){
  const [purchases,setPurchases]=useState(()=>getPurchases());
  const [tab,setTab]=useState("themes");
  const [trial,setTrial]=useState(null); // {id, remaining}
  ...
  useEffect(()=>{
    if(!trial)return;
    if(trial.remaining<=0){setTrial(null);onTryTheme(null);return;}
    const id=setTimeout(()=>setTrial(t=>t?{...t,remaining:t.remaining-1}:null),1000);
    return()=>clearTimeout(id);
  },[trial,onTryTheme]);

  const startTrial=(id)=>{
    setTrial({id,remaining:30});
    onTryTheme(id);
  };
    ...
    <div style={{position:"fixed",inset:0,background:t.bg,display:"flex",flexDirection:"column",padding:"max(env(safe-area-inset-top),14px) 0 max(env(safe-area-inset-bottom),16px)",boxSizing:"border-box",fontFamily:MONO,color:t.pri,overflow:"hidden"}}>
      ...
      <div style={{flex:1,overflowY:"auto",padding:"0 18px 16px",display:"flex",flexDirection:"column",gap:8}}>
        ...
        {tab==="themes"&&<>
          ...
          {trial&&<div style={{background:"#f59e0b22",border:"1.5px solid #f59e0b",borderRadius:10,padding:"8px 12px",marginBottom:8,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
            <span style={{fontSize:11,color:"#f59e0b",fontWeight:700,fontFamily:MONO}}>{'Trying '+THEMES[trial.id]?.name+' — '+trial.remaining+'s left'}</span>
            <Btn onTap={()=>{setTrial(null);onTryTheme(null);}} style={{padding:"3px 8px",borderRadius:6,border:"1px solid #f59e0b",background:"transparent",color:"#f59e0b",fontSize:9,fontWeight:700}}>End</Btn>
          </div>}
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
            {PREMIUM_THEMES.map(id=>{
              ...
              const isTrying=trial?.id===id;
              return(
                <div key={id} style={{borderRadius:14,overflow:"hidden",border:`2px solid ${isOwned?t.acc:isTrying?"#f59e0b":t.brd}`,background:pal.bg,display:"flex",flexDirection:"column"}}>
                  ...
                  <div style={{padding:"4px 8px 10px",display:"flex",flexDirection:"column",gap:4}}>
                    ...
                    {isOwned
                      ...
                      :<div style={{display:"flex",gap:4}}>
                        <Btn onTap={()=>mockBuy("theme",id)} style={{flex:1,padding:"4px 6px",borderRadius:8,background:`${pal.acc}22`,border:`1px solid ${pal.acc}`,color:pal.acc,fontSize:10,fontWeight:700}}>$0.59</Btn>
                        {!isTrying&&!trial&&<Btn onTap={()=>startTrial(id)} style={{flex:1,padding:"4px 6px",borderRadius:8,background:"#f59e0b22",border:"1px solid #f59e0b",color:"#f59e0b",fontSize:9,fontWeight:700}}>Try 30s</Btn>}
                      </div>
                    }
                  </div>
                </div>
...
export default function App(){
  ...
  const handleWin=(time,info)=>{
    ...
    // Milestone toasts
    const SOLVE_MILES=[10,25,50,100,250,500];
    const STREAK_MILES=[3,7,14,30,60,100];
    const prevTotal=(stats?.totalWins||0);
    const newTotal=(ns?.totalWins||0);
    const milestoneToasts=[];
    SOLVE_MILES.forEach(m=>{if(prevTotal<m&&newTotal>=m)milestoneToasts.push({type:"ach",icon:"🎯",label:`${m} Puzzles Solved!`,desc:"Milestone reached"});});
    STREAK_MILES.forEach(m=>{if((ns?.dStreak||0)===m)milestoneToasts.push({type:"ach",icon:"🔥",label:`${m} Day Streak!`,desc:"Keep it going"});});
    if(mistakes===0&&prevTotal>0){const prevFlawless=stats?.flawlessWins||0;if(prevFlawless===0)milestoneToasts.push({type:"ach",icon:"✨",label:"First Flawless!",desc:"No mistakes — perfect solve"});}

    // Toast sequence: achievements → goal badges → streak rewards → milestones
    const toastQueue=[
  ...
  return(
    <div style={{position:"fixed",inset:0,overflow:"hidden",background:t.bg}}>
      ...
      {screen==="store"      &&pg("store",       <Store t={t} onBack={()=>go("home")} onPurchase={p=>{setPurchases(p);if(p.noAds||p.allThemes&&p.noAds)chg("_noAds",true);}} onTryTheme={id=>{if(id){const saved=settings.theme;chg("theme",id);chg("_trialRestore",saved);}else{chg("theme",settings._trialRestore||"midnight");chg("_trialRestore",null);}}}/>)}
