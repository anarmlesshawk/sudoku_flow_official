...
// STORAGE
// ─────────────────────────────────────────────
const K={set:"sf_settings_v1",stats:"sf_stats_v2",hist:"sf_hist_v1",achv:"sf_achv_v1",arch:"sf_arch_v1",ob:"sf_ob_v1",goals:"sf_goals_v1",rewards:"sf_rewards_v1",weekly:"sf_weekly_v1",notif:"sf_notif_v1"};
...
function GameScreen({t,settings,difficulty:initDiff,savedState,seed:initSeed,onHome,onSave,onWin,onLoss,isDaily,isPractice,gameMode}){
  ...
  return(
    <div style={{position:"fixed",inset:0,background:gt.bg,backgroundImage:settings.bgPhoto?`url(${settings.bgPhoto})`:"none",backgroundSize:"cover",backgroundPosition:"center",display:"flex",flexDirection:"column",alignItems:"center",boxSizing:"border-box",fontFamily:MONO,color:gt.pri,touchAction:"manipulation",overflow:"hidden"}}>
      ...
      <div style={{width:"100%",height:50,background:gt.bgCard,borderBottom:`1px solid ${gt.brd}`,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,paddingTop:"max(env(safe-area-inset-top),0px)"}}>
        {/* Ad banner slot */}
      </div>
      ...
      <div style={{width:"100%",maxWidth:460,display:"flex",alignItems:"center",justifyContent:"space-between",flexShrink:0,padding:"8px 12px 4px"}}>
        ...
        <div style={{textAlign:"right",minWidth:70}}>
          {!isZen&&settings.showTimer&&<div style={{fontSize:"clamp(13px,3.5vw,16px)",fontWeight:700,color:gt.pri,marginBottom:4}}>{fmt(timer)}</div>}
          {isZen&&<div style={{fontSize:13,color:gt.mut,marginBottom:4}}>∞</div>}
          {!isFocus&&(
            <div style={{display:"flex",flexDirection:"column",alignItems:"flex-end",gap:4}}>
              <div style={{display:"flex",gap:5}}>
                {[1,2,3].map(i=><div key={i} style={{width:11,height:11,borderRadius:"50%",background:i<=mistakes?gt.err:gt.brd,boxShadow:i<=mistakes?`0 0 8px ${gt.err}`:"none",animation:i===mistakes&&mistakes>0?"shk .35s ease":"none"}}/>)}
              </div>
              <div style={{fontSize:11,fontWeight:700,color:mistakes>0?gt.err:gt.mut,fontFamily:MONO}}>{mistakes}/3 err</div>
            </div>
          )}
      ...
      <div style={{position:"relative",width:"min(96vw,calc(100vh - 280px))",height:"min(96vw,calc(100vh - 280px))",flexShrink:0,margin:"4px 0"}}>
        {/* Col indicators */}
      ...
      {/* Number pad */}
      <div style={{display:"flex",gap:"clamp(3px,1.2vw,6px)",width:"min(96vw,440px)",flexShrink:0,padding:"0 4px"}}>
      ...
      <div style={{display:"flex",gap:"clamp(3px,1.2vw,6px)",width:"min(96vw,440px)",flexShrink:0,paddingBottom:"max(env(safe-area-inset-bottom),10px)",padding:`0 4px max(env(safe-area-inset-bottom),10px)`,marginTop:6}}>
...
// ROOT APP
// ─────────────────────────────────────────────
const DEF_SET={theme:"midnight",dark:true,dynamicTheme:false,highlightMistakes:true,autoNotes:true,highlightSame:true,showTimer:true,pauseOnHide:true,soundOn:true,hapticsOn:true,allowBgAudio:false,singleTapClear:false,textSize:"medium",colorblind:false,boardReward:null,doNotDisturb:false,numFont:"rounded",bgPhoto:null,boardOpacity:100,opacityBgOnly:true};
