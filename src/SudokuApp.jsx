...
function GameScreen({t,settings,difficulty:initDiff,savedState,seed:initSeed,onHome,onSave,onWin,onLoss,isDaily,isPractice,gameMode}){
  ...
  return(
    <div style={{position:"fixed",inset:0,background:gt.bg,display:"flex",flexDirection:"column",alignItems:"center",paddingTop:"max(env(safe-area-inset-top),10px)",paddingBottom:"max(env(safe-area-inset-bottom),14px)",paddingLeft:8,paddingRight:8,boxSizing:"border-box",fontFamily:MONO,color:gt.pri,touchAction:"manipulation",overflow:"hidden",gap:6}}>
      ...
      {/* Board — explicit square size: smaller of 96vw or available vertical space */}
      <div style={{position:"relative",width:"min(96vw,calc(100vh - 220px))",height:"min(96vw,calc(100vh - 220px))",flexShrink:0}}>
        {/* Col indicators */}
          ...
          {board.map((row,r)=>row.map((val,c)=>{
            ...
            return(
              <div key={ck}
                ...
                {val!==0
                  ?<span style={{fontSize:numSz,fontWeight:600,color:nc,fontFamily:NUM,fontVariantNumeric:"tabular-nums",lineHeight:1,opacity:isFad?0:1,transition:"opacity .6s",animation:isFlsh&&!isFad?"shk .35s ease":"none",display:"block",textAlign:"center",width:"100%"}}>
                      {val}
        ...
        {[1,2,3,4,5,6,7,8,9].map(n=>{
          ...
          return(
            <Btn key={n} onTap={()=>{}} disabled={done||won||over}
              style={{flex:1,aspectRatio:"0.82",border:`1.5px solid ${done?gt.brd:notes?acc:gt.brdM}`,borderRadius:8,background:done?gt.bg:notes?`${acc}10`:gt.bgCard,color:done?gt.ghost:gt.pri,fontSize:padSz,fontWeight:600,fontFamily:NUM,fontVariantNumeric:"tabular-nums",position:"relative",display:"flex",alignItems:"center",justifyContent:"center",minWidth:0}}
      ...
      <div style={{display:"flex",gap:5,width:"min(96vw,440px)",flexShrink:0}}>
        ...
        {!isDaily&&!isEndless&&!isPractice&&(
          ...
          }} style={aBt(newPend,"#f59e0b")}>{newPend?"Sure?":"↺ New"}</Btn>
        )}
