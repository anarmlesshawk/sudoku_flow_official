    ...
    <div style={{position:"fixed",inset:0,background:gt.bg,display:"flex",flexDirection:"column",alignItems:"center",paddingTop:"max(env(safe-area-inset-top),10px)",paddingBottom:"max(env(safe-area-inset-bottom),14px)",paddingLeft:8,paddingRight:8,boxSizing:"border-box",fontFamily:MONO,color:gt.pri,touchAction:"manipulation",overflow:"hidden",gap:6}}>
      ...
      <div style={{position:"relative",flexGrow:1,flexShrink:1,minHeight:0,aspectRatio:"1/1",width:"min(94vw,100%)",maxWidth:"calc(100vh - 200px)",alignSelf:"center"}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(9,1fr)",width:"100%",height:"100%",borderRadius:8,overflow:"hidden",border:`2px solid ${gt.brdM}`,boxSizing:"border-box",position:"relative",...(settings.boardReward?getRewardStyle(settings.boardReward,acc):{boxShadow:`0 8px 32px rgba(0,0,0,0.18)`})}}>
          {/* All grid lines drawn as overlays — zero cell borders so all 9 cols are pixel-perfect equal */}
          {/* Thin vertical lines at cols 1,2,4,5,7,8 */}
          {[1,2,4,5,7,8].map(i=>(
            <div key={`vt${i}`} style={{position:"absolute",top:0,bottom:0,left:`${i*100/9}%`,width:1,background:gt.brd,zIndex:2,pointerEvents:"none",transform:"translateX(-0.5px)"}}/>
          ))}
          {/* Thick vertical lines at cols 3,6 (box boundaries) */}
          {[3,6].map(i=>(
            <div key={`vb${i}`} style={{position:"absolute",top:0,bottom:0,left:`${i*100/9}%`,width:2,background:gt.brdM,zIndex:3,pointerEvents:"none",transform:"translateX(-1px)"}}/>
          ))}
          {/* Thin horizontal lines at rows 1,2,4,5,7,8 */}
          {[1,2,4,5,7,8].map(i=>(
            <div key={`ht${i}`} style={{position:"absolute",left:0,right:0,top:`${i*100/9}%`,height:1,background:gt.brd,zIndex:2,pointerEvents:"none",transform:"translateY(-0.5px)"}}/>
          ))}
          {/* Thick horizontal lines at rows 3,6 (box boundaries) */}
          {[3,6].map(i=>(
            <div key={`hb${i}`} style={{position:"absolute",left:0,right:0,top:`${i*100/9}%`,height:2,background:gt.brdM,zIndex:3,pointerEvents:"none",transform:"translateY(-1px)"}}/>
          ))}
